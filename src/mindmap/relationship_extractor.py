...
if __name__ == "__main__":
    ...
    test_data = [
        ...
        "Artificial neural networks are inspired by biological brain structure",
        "Supervised learning requires labeled training data for model training",
        "Natural language processing helps computers understand human text",
        "Text classification is a common NLP task for document categorization",
    ...
    # Process document
    result = system.process_document(test_data, document_type="text")
    
    # Extract relationships
    enhanced_result, summary = extract_and_enhance_relationships(
        result['clustering'], 
        result['embeddings'], 
        result['texts'],
    ...
    # Enhance mindmap
    extractor = RelationshipExtractor()
    enhanced_mindmap = extractor.enhance_mindmap_with_relationships(result, enhanced_result)
    
    # Display results
    print(f"\n📊 Relationship Summary:")
    print(f"   Total relationships: {summary['total_relationships']}")
    print(f"   Average per cluster: {summary['average_per_cluster']:.1f}")
    
    if summary.get('strongest_relationship'):
        print(f"   Strongest relationship: {summary['strongest_relationship']['confidence']:.3f}")
        print(f"      {summary['strongest_relationship']['source'][:50]}...")
        print(f"   → {summary['strongest_relationship']['target'][:50]}...")
    
    print(f"\n🌳 Enhanced Mindmap Structure:")
    for branch in enhanced_mindmap['mindmap']['branches']:
        print(f"   Branch: {branch['title']} ({branch['size']} concepts)")
        print(f"      Relationships: {branch['relationship_count']}")
        print(f"      Density: {branch['relationship_density']:.3f}")
        
        if branch['relationships']:
            print(f"      Sample relationships:")
            for rel in branch['relationships'][:2]:  # Show first 2
                print(f"         Concept {rel['source_index']} → Concept {rel['target_index']} (confidence: {rel['confidence']:.3f})")